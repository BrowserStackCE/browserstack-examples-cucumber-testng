...
node {
  try {
    ...
    stage('Pull code from Github') {
        dir('test') {
            git branch: 'main', changelog: false, poll: false, url: 'https://github.com/browserstack/browserstack-examples-cucumber-testng'
        }
    }
    ...
    stage('Run Test') {
        browserstack(credentialsId: "${params.BROWSERSTACK_USERNAME}") {
            ...
            withEnv(['BROWSERSTACK_USERNAME=' + user]) {
                ...
                cd test
                mvn -Ddataproviderthreadcount=${PARALLELISM} clean test -P ${TEST_TYPE} '''
            }
        }
    }
    stage('Generate Report'){
          ...
          allure([
                   ...
                   results: [[path: 'target/allure-results'], [path: '**/test/allure-results']]
                    ])
            }
